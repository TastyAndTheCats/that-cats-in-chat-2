...
async fn send_response_or_run_response_fn(
    ...
    if r.response_fn.is_some() {
        ...
        responder::run(client, r, msg, command).await;
    } else {
...
fn format_response(r: &TwitchResponder, msg: &PrivmsgMessage, command: &str) -> String {
    insert_data_in_response(r, msg, command)
}

/// Rwplaces text variables (no format yet) with the real data where possible
fn insert_data_in_response(r: &TwitchResponder, msg: &PrivmsgMessage, command: &str) -> String {
