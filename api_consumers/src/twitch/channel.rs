...
pub async fn set_game(channel_id: &str, title: String) -> Result<Response, Error> {
...
pub async fn set_title(channel_id: &str, title: String) -> Result<Response, Error> {
    twitch::patch(
        &format!(
            " https://api.twitch.tv/helix/channels?broadcaster_id={}",
            channel_id
        ),
        Some(vec![("title", title)]),
        Some(channel_id.parse::<i32>().unwrap_or(0)),
    )
    .await
}
